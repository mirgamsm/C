...
*  by Mateusz Mirga                                                                                                    *                                                   
*                                                                                                                      *
*   ---If ran, the program will encrypt all .txt files within the directory of the program using the Tiny Encryption   *
*       Algorithm                                                                                                      *
*   --- To decrypt simply run the file again                                                                           *
*                                                                                                                      *   
*                                                                                                                      *
*                                                                                                                      *
*                                                                                                                      *
...
char BUFFER[1000];                                  //Buffer for loading and unloading data from files                    
char stamp[] ="TrojanSnake;";                       //String used for checking if file has already been encrypted
...
unsigned char L;                                    //variables part of TEA 
unsigned char R;
unsigned int delta =0x9e3779b9;

unsigned char iV ='V';                              //Used to initalize CBC

                                                    //Functions                                            
int has_txt_extension(char const *name);
void encrypt(int size);
void decrypt(int size);
void CBC(int position);
int encryption();
...
int encryption()
{
 struct dirent *de;                              // Pointer for directory entry
    ...
    while ((de = readdir(dr)) != NULL)              //cycle throught files loop
        if(has_txt_extension(de->d_name)==0){       
            ...
            fclose(fptr); 
            if(BUFFER[i-1]==';'){                   //Checks if file has already been encrypted
                decrypt(i-12);            
                fptr =fopen(name, "w");
                if(BUFFER[i-13]=='V')
                    i--;           
                ...
                if(i%2!=0){                        //Checks if there is an even number of chars, if not adds V
                    BUFFER[i]=iV;
                    i++;
                }                           
                encrypt(i);
                fptr =fopen(name, "w");
                for(int j =0;j<i;j++)
                    fputc(BUFFER[j],fptr);         //Overwrites file with encrypted text
                for(int j=i;j<i+12;j++)
                    fputc(stamp[j-i], fptr);       //Stamps file
                fclose(fptr);
            }
        }
closedir(dr);    
return 0;
                                                    ...
                                                    // this fun checks if the a given file name has a .txt extension  
int has_txt_extension(char const *name){
    size_t len =strlen(name);
    if(len >4) 
     return strcmp(name + len -4,".txt");
}
                                                    //this function encrypts the Buffer load
void encrypt(int size){ 
    printf("\nEncrypting\n"); 
    for(int j =0;j<size;j+=2){
        ...
        for(int i =0;i<32;i++){
            sumE =sumE+delta;
            L =L+(((R<<4)+k0)^(R+sumE)^((R>>5)+k1));
            R =R+(((L<<4)+k2)^(L+sumE)^((L>>5)+k3));
        }
        BUFFER[j] =L;
        BUFFER[j+1] =R;
    }
}
                                                    //this function decrypts the Buffer load
 void decrypt(int size){
     printf("\nDecrypting\n");     
    for(int j =size;j>=0;j-=2){
        ...
        R =BUFFER[j+1];
     for(int i =0;i<32;i++){
         R =R-(((L<<4)+k2)^(L+sumD)^((L>>5)+k3));
         L =L-(((R<<4)+k0)^(R+sumD)^((R>>5)+k1));
         sumD =sumD -delta;
     }
        BUFFER[j] =L;
        BUFFER[j+1] =R;
